...
OPENSSL_CFLAGS := $(shell pkg-config --cflags openssl 2>/dev/null || echo "-I/usr/local/opt/openssl/include")
...
CFLAGS += $(LIBMILL_CFLAGS) $(OPENSSL_CFLAGS) 
LDFLAGS = $(LIBMILL_LIBS) $(OPENSSL_LIBS)
